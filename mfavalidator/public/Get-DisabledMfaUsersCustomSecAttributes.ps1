function Get-DisabledMfaUsersCustomSecAttributes {
    [cmdletbinding()]
    Param (
        [Parameter(Mandatory)]
        [array]$userList,
        [Parameter(Mandatory=$false)]
        [array]$usersToExclude = @("On-Premises Directory Synchronization Service Account"),
        [Parameter(Mandatory)]
    ...
    Begin {
        ...
        $i = 0
        $batchRequest = $null
        $usersWithIncorrectAttributes = [System.Collections.Generic.List[string]]::new()
    }
    Process {
            ...
            if ($batchRequest.requests.Count -eq 20) {
                ...
                Try {
                    ...
                    foreach ($response in $resp.responses) {
                        ...
                        if ($mfaStatus -eq $false -and $dateFlagged -or $displayName -in $usersToExclude) {
                            continue
                        }
                        else {
                            $usersWithIncorrectAttributes.Add($response.body.Id)
                        }
                    }
                }
                ...
                if ($remainder -eq 0) {
                    ...
                    for ($x = 1; $x -lt ($remainder + 1); $x++) {
                        $request = [pscustomobject][ordered]@{
                            "id"     = $x
                            "method" = "GET"
                            "url"    = "users/$($userList[$i])?`$select=customSecurityAttributes,Id,displayName"
                        }
    ...
    End {
        Return $usersWithIncorrectAttributes
    }
}