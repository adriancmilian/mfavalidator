function Clear-CustomSecAttributes {
    [cmdletbinding()]
    param (
        [Parameter(Mandatory)]
        [array]$userList,
    ...
    Begin {
        # Create variables to be used within the pipeline
        $uri = "https://graph.microsoft.com/beta/`$batch"
        $userCount = $userList.Count
        $i = 0
        $batchRequest = $null
        # Create hashtable with specific attributes we will be setting for the users
        $attset = @{
    ...
    Process {
        # Begin while loop to create and invoke json batch requests to the graph api
